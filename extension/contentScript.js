(() => {
    ...
    function isLowIntentControl(control) {
        ...
        if (type === "search" || el.getAttribute("role") === "searchbox")
            return true;
        if (container && containsAny(intentText, ["search", "query", "keyword", "find"]))
            return true;
        if (tag === "input" && containsAny(intentText, ["search", "query", "keyword"]))
            return true;
        return false;
    }
    function getControls() {
        return Array.from(document.querySelectorAll("input, textarea, select")).filter((control) => {
            ...
            if (control.closest("#curion-root"))
                return false;
            if (["hidden", "submit", "button", "reset", "file", "image", "password", "search"].includes(type))
                return false;
            if (control.disabled || ("readOnly" in control && control.readOnly))
                return false;
            if (isLowIntentControl(control))
                return false;
    ...
    function shouldOfferAutoFill(analysis) {
        if (!analysis || analysis.mappedCount === 0)
            return false;
        if (analysis.fieldCount >= 2)
            return true;
        if ((analysis.mappings || []).filter((entry) => entry.mapping).length >= 2)
            return true;
    ...
    function fillReturnedMappings(mappings) {
        ...
        for (const entry of mappings || []) {
            ...
            if (!field || !mapping?.value)
                continue;
            if (!mappingCompatibleWithField(field, mapping))
                continue;
            const control = controls[field.index] ||
    ...
    function isElementVisible(element) {
        if (!(element instanceof HTMLElement))
            return false;
        if (element.closest("#curion-root"))
            return false;
    ...
    function buttonLabel(element) {
        ...
        return (element.textContent ||
            ...
            element.id ||
            "selected action").replace(/\s+/g, " ").trim();
    }
    function bestForm() {
    ...
    function scoreWorkflowAction(element, preferredForm) {
        ...
        if (!["button", "input", "a"].includes(tag) && role !== "button")
            return 0;
        if (tag === "input" && !["submit", "button"].includes(type))
            return 0;
        if (type === "reset" || type === "file" || type === "image")
            return 0;
        ...
        if (type === "submit")
            score += 25;
        if (preferredForm && element.closest("form") === preferredForm)
            score += 18;
        if (hasActionTerm(text, ["save and next", "save next", "submit and next", "submit next", "next step"]))
            score += 55;
        if (hasActionTerm(text, ["submit", "send", "continue", "next", "proceed", "finish", "complete", "done"]))
            score += 42;
        if (hasActionTerm(text, ["save"]))
            score += 24;
        return score;
    }
    ...
    function emptyRequiredControls(root) {
        return Array.from(root.querySelectorAll("input, textarea, select")).filter((control) => {
            if (!(control instanceof HTMLInputElement || control instanceof HTMLTextAreaElement || control instanceof HTMLSelectElement))
                return false;
            if (!control.required || !isElementVisible(control))
                return false;
    ...
    function submitBestForm() {
        ...
        const submitter = target.querySelector('button[type="submit"], input[type="submit"], button:not([type])');
  ...
  `;
    ...
    async function maybeAutoFill() {
        ...
        if (promptSignature === lastPromptSignature)
            return;
        lastPromptSignature = promptSignature;
        renderCurionPrompt(analysis, analysis.mappedCount
